/**
 * kelola_kategori.js - Admin UMKM Culinary's NTT
 * Berisi helper data global (AdminDB, render sidebar) + logika khusus halaman kelola_kategori.html.
 */
...
/* ===== Logika khusus halaman kelola_kategori.html (dipindahkan dari <script> inline) ===== */
renderAdminSidebar('kategori');

    function renderCategories() {
      const categories = AdminDB.getCategories();
      const products = AdminDB.getProducts();
      const tbody = document.getElementById('category-table-body');
      document.getElementById('total-cat-count').innerText = `Total: ${categories.length}`;

      tbody.innerHTML = categories.map(cat => {
        const prodCount = products.filter(p => p.category === cat.name).length;
        return `
          <tr>
            <td><strong style="color: #1c1917;">${cat.name}</strong></td>
            <td style="text-align: center;">
              <span style="background: #f5f5f4; padding: 2px 8px; border-radius: 6px; font-size: 12px; font-weight: 600;">
                ${prodCount} produk
              </span>
            </td>
            <td style="text-align: right;">
              <div style="display: inline-flex; gap: 6px;">
                <button class="btn btn-secondary btn-sm" onclick="editCategory('${cat.id}', '${cat.name}')">Edit</button>
                <button class="btn btn-danger btn-sm" onclick="deleteCategory('${cat.id}', '${cat.name}')">Hapus</button>
              </div>
            </td>
          </tr>
        `;
    ...
    function handleAddCategory(e) {
      ...
      AdminDB.saveCategories(categories);
    ...
    function deleteCategory(id, name) {
      const products = AdminDB.getProducts();
      const prodCount = products.filter(p => p.category === name).length;

      if (prodCount > 0) {
        if (!confirm(`Kategori "${name}" memiliki ${prodCount} produk. Yakin ingin menghapus?`)) {
          return;
        }
      } else {
      ...
      let categories = AdminDB.getCategories();
